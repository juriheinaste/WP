		...
		if ( ( !$footer_content = get_transient( 'fabulous_fluid_footer_content' ) ) ) {
			...
			$footer_content = '
				<span class="copyright">' . sprintf(
							_x( 'Copyright &copy; %1$s %2$s %3$s' ,
								'1: Year, 2: Site Title with home URL 3: Privacy Policy Link',
								'fabulous-fluid'
							),
							esc_attr( date_i18n( __( 'Y', 'fabulous-fluid' ) ) ),
							'<a href="'. esc_url( home_url( '/' ) ) .'">'. esc_attr( get_bloginfo( 'name', 'display' ) ) . '</a>', get_the_privacy_policy_link()
